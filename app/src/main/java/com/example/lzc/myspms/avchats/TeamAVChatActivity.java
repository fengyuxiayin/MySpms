...
//        activeCallingNotifier(false);
        setChatting(false);
        Log.e(TAG, "onDestroy: "+TeamAVChatProfile.sharedInstance().isTeamAVChatting() );
    ...
    private void onIntent() {
        ...
        Log.e(TAG, "onIntent: "+accounts.size() );
    ...
    private void findLayouts() {
        ...
        voiceMuteButton.setVisibility(View.INVISIBLE);
    }
    ...
    private void showReceivedCallLayout() {
        ...
        textView.setText(teamName + "发起的视频会议");
    ...
    private void hangup() {
        ...
        try {
            ...
            Log.e(TAG, "hangup: leaveroom" +roomId);
            AVChatManager.getInstance().leaveRoom2(roomId, new AVChatCallback<Void>() {
                ...
                @Override
                public void onException(Throwable exception) {
                    Log.e(TAG, "onException: "+ exception.getMessage()+ exception.getCause() );
                }
            });
    ...
    private void startTimerForCheckReceivedCall() {
        mainHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                int index = 0;
                for (TeamAVChatItem item : data) {
                    if (item.type == TeamAVChatItem.TYPE.TYPE_DATA && item.state == TeamAVChatItem.STATE.STATE_WAITING) {
...
//                finish();
    ...
    private void initRecyclerView() {
        ...
        for (String account : accounts) {
            Log.e(TAG, "initRecyclerView: "+account );
            if (account.equals(AVChatKit.getAccount())) {
                continue;
            }
            data.add(new TeamAVChatItem(TeamAVChatItem.TYPE.TYPE_DATA, teamId, account));
        }

        TeamAVChatItem selfItem = new TeamAVChatItem(TeamAVChatItem.TYPE.TYPE_DATA, teamId, AVChatKit.getAccount());
