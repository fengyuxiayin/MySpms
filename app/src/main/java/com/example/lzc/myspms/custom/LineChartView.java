...
/**
 * 自定义折线图
 * Created by lizhichao on 18/06/12.
 */
public class LineChartView extends View {
    //xy坐标轴颜色
    private int xylinecolor = getResources().getColor(R.color.homepage_text_press);
    //xy坐标轴宽度
    private int xylinewidth = dpToPx(1);
    //xy坐标轴文字颜色
    private int xytextcolor = getResources().getColor(R.color.homepage_text_press);
    //xy坐标轴文字大小
    private int xytextsize = spToPx(12);
    //折线图中折线的颜色
    private int linecolor = getResources().getColor(R.color.homepage_text_press);
    //x轴各个坐标点水平间距
    private int interval = dpToPx(80);
    //背景颜色
    private int bgcolor = getResources().getColor(R.color.transparent);
...
//        linePaint.setStyle(Paint.Style.STROKE);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        if (changed) {
            ...
            int dp2 = dpToPx(4);
            int dp3 = dpToPx(15);
            xOri = (int) (dpToPx(4) + textYWdith + dpToPx(4) + xylinewidth);//dp2是y轴文本距离左边，以及距离y轴的距离
//            //X轴文本最大高度
            ...
            yOri = (int) (height - dpToPx(4) - textXHeight - dpToPx(15) - xylinewidth);//dp3是x轴文本距离底边，dp2是x轴文本距离x轴的距离
            xInit = interval + xOri;
...
//        drawBorkenLineShadow(canvas);
    ...
    private void drawFloatTextBox(Canvas canvas, float x, float y, Double text) {
        ...
        canvas.drawText(text + "", x - rect.width() / 2, y + dp6 +dp18/2+ (rect.height()) / 2, linePaint);
    }
    /**
     *
     *@desc 绘制折线下面的阴影
    ...
    */
private void drawBorkenLineShadow(Canvas canvas){
    linePaint.setStyle(Paint.Style.STROKE);
    linePaint.setColor(linecolor);
    //绘制折线
    Path path = new Path();
    ...
    private void drawXY(Canvas canvas) {
        ...
        for (int i = 0; i < yValue.size(); i++) {
            ...
            canvas.drawText(text, 0, text.length(), xOri - xylinewidth - dpToPx(4) - rect.width(), yOri - yLength * i + rect.height() / 2, xyTextPaint);
        }
        //绘制X轴坐标
...
//                    canvas.drawRoundRect(x - xValueRect.width() / 2 - dpToPx(3), yOri + xylinewidth + dpToPx(1), x + xValueRect.width() / 2 + dpToPx(3), yOri + xylinewidth + dpToPx(2) + xValueRect.height() + dpToPx(2), dpToPx(2), dpToPx(2), xyTextPaint);
    ...
    public void setValue(Map<String, Double> value, List<String> xValue, List<Integer> yValue) {
        this.value = value;
        this.xValue = xValue;
        this.yValue = yValue;
        for (int i = 0; i < value.size(); i++) {
            Log.e("hhahahhahhahahha", "setValue: " +value.size());
        }
        invalidate();
    }
